...
auhor: angus brooks -- cleaned and refactored ai
"""
...
# Static file routes
STATIC_ROUTES = [
    ('/public/css/<filename>', '/static/ex/css/<filename>', './public/css'),
    ('/public/images/<filename>', '/static/ex/images/<filename>', './public/images'),
    ('/public/js/<filename>', '/static/ex/js/<filename>', './public/js'),
]


def create_static_route(route1, route2, root_dir):
...
for route1, route2, root in STATIC_ROUTES:
    create_static_route(route1, route2, root)
...
class BookmarkManager:
    ...
    def insert_bookmark(user_id, title, url):
        ...
        # delete this -- bookmark_id, place_id = BookmarkManager.get_next_ids()
...
def render_main_view(user_id=None, err_obj=None, init=True):
    ...
    return exec_page(request, user_id, user_name, err_obj, init)
...
def tab_table_view():
    ...
    error_code = request.get_cookie("Error")
    error_url = request.get_cookie("Error_url")
    
    err_obj = Error(int(error_code), error_url) if error_code else None
    
    response.delete_cookie("Error")
    response.delete_cookie("Error_url")
    
    return exec_page(request, user_id, user_name, err_obj, init=False)
...
def add_web_mark():
    ...
    if BookmarkManager.check_duplicate(user_id, url):
        response.set_cookie("Error", str(150), path=APP_COOKIE_PATH, expires=int(time.time()) + FIVE_DAY_SECONDS)
        response.set_cookie("Error_url", url, path=APP_COOKIE_PATH, expires=int(time.time()) + FIVE_DAY_SECONDS)
        return render_main_view(user_id, Error(150, url))
